...
document.addEventListener("DOMContentLoaded", function () {
	...
	// Восстанавливаем рейтинг из локального хранилища
	restoreRatings();
});
...
function generateStarRating(index) {
	...
	for (let i = 1; i <= 5; i++) {
		stars += `
            <input type="radio" id="star${i}-${index}" name="rating-${index}" value="${i}" onclick="saveRating(${index}, ${i})">
            <label for="star${i}-${index}" class="star">★</label>`;
	}
	return `<div class="stars">${stars}</div>`;
}

// Сохранение рейтинга в локальное хранилище
function saveRating(heroIndex, rating) {
	localStorage.setItem(`hero-${heroIndex}-rating`, rating);
}

// Восстановление рейтингов из локального хранилища
function restoreRatings() {
	heroes.forEach((_, index) => {
		let savedRating = localStorage.getItem(`hero-${index}-rating`);
		if (savedRating) {
			document.querySelector(`#star${savedRating}-${index}`).checked = true;
		}
	});
}
